# -*- coding: utf-8 -*-
"""IJK 2028 予算モデル v10 / 2026-09-24 各日150人規模。
...
VERSION = "v10_20260924"
REVISION = "追加縮小の検討案に一本化。全日100人＋前半50人＋後半50人、運営15人別枠。小部屋20室は一人6・二人7・三人7室の41人。三人2500円・二人3300円・一人5000円/人泊。一人利用は35歳以下・全日・TEJO会員限定。大本総額100万円と第5日日帰りは条件付き計算"
...
def scenario(label="現行：追加縮小の検討案", *,
    ...
    return {
        ...
        "assumptions": {
            ...
            "oomoto_total_notice": "総額方式は宿泊費をまとめ、明示した共通費だけ除く。食材料・調理済み食事代は別。100万円は主催側の見通しで正式見積ではない。運営宿泊・対象日・会場・音響・通信・食堂運営の範囲を確認する。未指定時は個別積算",
...
def build_report():
    ...
    specs = [
        ("current_plan", "現行の追加縮小検討案：個別積算、共通費300万・TEJO等100万・遠足支援30万", {}),
        ("oomoto_total100_all", "範囲確認後：宿泊と会場・音響・通信・食堂運営を大本総額100万円にまとめる", {
            "oomoto_total_yen": 1000000, "common_covered_yen": covered}),
        ("oomoto_total100_external_audio", "範囲確認後：大本総額100万円、音響60万円は総額外に残る場合", {
            "oomoto_total_yen": 1000000, "common_covered_yen": covered-600000}),
        ("oomoto_total120_all", "金額確認：同じ範囲の大本総額が120万円の場合", {
            "oomoto_total_yen": 1200000, "common_covered_yen": covered}),
        ("current_day5_40", "第5日の受付・会場内企画を用意でき、40人を追加受付する場合", {"paid_days": 4}),
        ("current_food550", "食事単価550円の場合", {"meal_price": 550}),
    ...
    return {
        ...
        "provisional": {
            ...
            "oomoto_100man_estimate": "主催側の見通し。宿泊と会場・音響・通信・食堂運営を合計100万円に含む場合を試算。食事代は別",
...
if __name__ == "__main__":
    ...
    target = args.output_dir / "IJK2028_予算モデル_v10_計算結果_20260924.json"
