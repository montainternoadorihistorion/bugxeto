# -*- coding: utf-8 -*-
"""IJK 2028 予算モデル v9 / 2026-09-23 各日150人規模。
一般参加は全日100人・前半50人・後半50人の実人数200人。運営15人は別枠。
前半・後半は各4日3泊、各々全日料金の70%。第4夜は短期参加者が泊まらない計算。
宿泊追加はベッド付き相部屋2500円/人泊、一人専有5000円/人泊。35歳以下・全日7泊のみ。
小部屋19室を1:2:3人利用でほぼ均等にする方針から、6室・6室・7室、計39人を仮置き。
実ベッド定員・予約数は未確認。原資料のLUMO227人と部屋数を単純に足さない。
音響60万・Wi-Fi10万・暑さ対策約5万は予算目安、縮小案全体は検討中。
大本宿泊・施設合計100万円は条件付き比較。施設原価と参加者向け料金を分ける。
...
VERSION = "v9_20260923"
REVISION = "全日100人＋前半50人＋後半50人、各日約150人・実人数200人。運営15人別枠。小部屋19室は一人6室・二人6室・三人7室で39人。相部屋2500円/人泊・一人専有5000円/人泊。音響60万・通信10万・暑さ約5万を目安。宿泊施設合計100万は条件付き比較"
...
def private_addition_yen(age, period="full", *, actual_nights=None, addition_night=5000):
...
def shared_bed_addition_yen(age, period="full", *, actual_nights=None, addition_night=2500):
    """ベッド付き相部屋の追加額。主催側確認済みの35歳以下・全日7泊が対象。"""
    return private_addition_yen(age, period, actual_nights=actual_nights, addition_night=addition_night)
...
def scenario(label="150人・8日間の暫定基準案", *,
             ...
             private_youth_full=6, private_youth_first=0, private_youth_second=0, private_room_limit=19,
             shared_bed_youth_full=33, shared_beds_per_room=2, shared_bed_addition_night=2500,
             shared_room_occupancies=(2,2,2,2,2,2,3,3,3,3,3,3,3),
    ...
    宿泊差引き2000円、ベッド相部屋追加2500円、一人専有追加5000円は参加者向け料金。
    施設宿泊原価1000円は別。相部屋追加と一人専有追加を同じ人に重ねない。
    ...
    for name, value in (("全日暑さ対策費", heat_full_yen), ("一人利用1泊追加額", private_addition_night), ("個室追加原価", private_extra_cost_night),
                        ("ベッド付き相部屋1泊追加額", shared_bed_addition_night), ("ベッド付き相部屋追加原価", shared_bed_extra_cost_night),
    ...
    if shared_beds_per_room < 1:
        raise ValueError("相部屋の1室人数の仮定は1以上")
    if shared_room_occupancies is not None:
        if not isinstance(shared_room_occupancies, (tuple, list)) or any(type(n) is not int or n < 1 for n in shared_room_occupancies):
            raise ValueError("相部屋ごとの利用人数は正の整数の配列")
    ...
    income = {
        ...
        "shared_bed_youth": shared_bed_youth_full*shared_bed_addition_yen(BASE_AGE_KEY, addition_night=shared_bed_addition_night),
    ...
    return {
        ...
        "assumptions": {
            ...
            "shared_bed_addition_basis": f"ベッド付き相部屋は全日7泊{shared_bed_addition_night*7}円追加。一人専有の追加とは重ねない。対象は主催側確認済みの35歳以下・全日7泊",
...
def build_report():
    ...
    specs = [
        ("baseline", "比較用の暫定基準：音響80万円等を維持、全日100人・前後半各50人・運営15人", {}),
        ("previous_costs_at_150", "比較：各日150人規模で以前の支出枠を維持", {"fixed": 4500000, "heat_full_yen": 1000}),
        ("lean_proposal", "追加縮小の検討案：共通費300万・TEJO等100万・遠足支援30万", lean),
        ("lean_private_10", "追加縮小案・ベッド付き相部屋10人", {**lean, "shared_bed_youth_full": 10, "private_youth_full": 0, "shared_room_occupancies": None}),
        ("lean_private_0", "追加縮小案・ベッド追加利用0人", {**lean, "shared_bed_youth_full": 0, "private_youth_full": 0, "shared_room_occupancies": None}),
        ...
        ("lean_private_cost_1000", "追加縮小案・ベッド追加原価1000円/人泊", {**lean, "shared_bed_extra_cost_night": 1000, "private_extra_cost_night": 1000}),
        ...
        ("lean_120", "追加縮小案・全日80人＋前後半各40人、運営15人", {**lean, "full": 80, "first": 40, "second": 40,
            "age_counts": {"full": {"16_or_under": 4, "17_24": 32, "25_35": 32, "36_plus": 12},
                           "first": {"16_or_under": 2, "17_24": 16, "25_35": 16, "36_plus": 6},
                           "second": {"16_or_under": 2, "17_24": 16, "25_35": 16, "36_plus": 6}}}),
        ("lean_combined", "追加縮小案・会員50%・相部屋10人・食事550円・日帰り2日", {
            **lean, "nonmember_share": .5, "shared_bed_youth_full": 10, "private_youth_full": 0, "shared_room_occupancies": None, "meal_price": 550, "paid_days": 2}),
    ]
    ...
    # 実ベッド数や販売数の予測ではなく、19室を異なる形で使う比較例。
    specs.extend([
        ("bedrooms_double", "配置仮例：19室を2人相部屋、計38人", {**lean, "shared_bed_youth_full": 38, "private_youth_full": 0, "shared_room_occupancies": None}),
        ("bedrooms_triple", "配置仮例：19室を3人相部屋、計57人", {**lean, "shared_bed_youth_full": 57, "shared_beds_per_room": 3, "private_youth_full": 0, "shared_room_occupancies": None}),
        ("bedrooms_single", "配置仮例：19室を一人専有、計19人", {**lean, "shared_bed_youth_full": 0, "private_youth_full": 19, "shared_room_occupancies": None}),
        ("bedrooms_mixed", "配置仮例：14室を2人相部屋、5室を一人専有、計33人", {**lean, "shared_bed_youth_full": 28, "private_youth_full": 5, "shared_room_occupancies": None}),
    ])
    rows = []
    ...
    return {
        ...
        "user_confirmed": {"ordinary_full": 100, "ordinary_first": 50, "ordinary_second": 50, "ordinary_registration": 200, "daily_ordinary_target": 150, "days": 8, "staff_is_separate": True,
            "observed_bedroom_rooms": 19, "private_rooms_include_annex": True,
            "bedroom_rooms_by_building": {"lumo": 4, "annex": 15},
            "private_room_counts_are_approximate": True,
            "private_room_count_basis": "主催側の見学後の内訳はLUMO館4室・別館15室程度。最新説明により基本は相部屋利用。19室を19人専用の一人部屋とは扱わない",
            "lodging_surcharge_proposal_yen_per_person_night": {"shared_bed": 2500, "exclusive_room": 5000},
            "both_bed_plans_eligible_ages": "35歳以下", "both_bed_plans_eligible_period": "全日7泊のみ",
            "bedroom_capacity_comparison_basis": "主催側は一人・二人・三人利用を室数1:1:1で仮置きする方針。19室の整数配分を6・6・7室とする。実ベッド数は未確認",
            "bedroom_room_ratio": [1,1,1],
        ...
        "provisional": {"staff_full": 15, "age_counts": [10,80,80,30],
            "hotel_full": 15, "hotel_first": 7, "hotel_second": 8, "paid_private_people": 6, "paid_shared_bed_people": 33,
            "bedroom_room_counts": {"single":6,"double":6,"triple":7},
            "uniform_comparison_beds_per_room": 2, "actual_bed_count_is_confirmed": False,
            "bed_plan_eligibility_notice": "主催側が両方とも35歳以下・全日参加者向けを維持すると回答済み",
            "free_day_person_days": 300, "onsite_capacity_document_total": 227,
            "capacity_is_authorized_for_2028": False,
            "room_allocation_notice": "主催側の1:1:1方針から一人6室・二人6室・三人7室を仮置きし39人。ベッド実数・取り置き・販売数は未確定。227人へ別枠で39人・19室を足さない",
...
if __name__ == "__main__":
    ...
    target = args.output_dir / "IJK2028_予算モデル_v9_計算結果_20260923.json"
